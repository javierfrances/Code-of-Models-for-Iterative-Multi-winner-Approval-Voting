...
if __name__ == "__main__":
    ...
    print("\nStarting Simulation")
    print("-------------------\n")
    startN = int(input("Enter the initial n value: "))
    endN = int(input("Enter the final n value: "))
    stepInN = int(input("Enter the step in n: "))

    if(startN > endN):
        print("Initial n cannot be higher than final n!")
        exit()
    ...
    variablesToChange = []
    if (h == 1):
        heuristics.extend([1 for j in range(startN)])
        thresholds = input("Enter threshold for the heuristic [0,1]: ").split()
        variablesToChange = [float(item) for item in thresholds]
    elif (h == 2):
        heuristics.extend([2 for j in range(startN)])
    elif (h == 3):
        heuristics.extend([3 for j in range(startN)])
        upToJs = input("How many changes (J)? ").split()
        for item in upToJs:
            if(int(item) > len(C)):
        ...
        heuristics.extend([4 for j in range(startN)])
    elif (h == 5):
        print("not done")
        exit()
    ...
    for item in variablesToChange:
        ...
        for simN in range(startN, endN+stepInN, stepInN):
            simulations.append(simN)
            if(not first):
                for z in range(stepInN):
                    heuristics.append(h)
            first = False
            f.write("\nTESTING WITH " + str(simN)+ " VOTERS:\n")
            f.write("----------------------\n")
            n = simN
    ...
    if(not len(variablesToChange) == 0):
        ...
        for item in averagesDPOALists:
            ...
            plt.title('How average dPOA of a game increases with N:')
        ...
        for item in averagesTimesLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How average time of a game increases with N:')
            plt.xlabel('Number of voters')
        ...
        for item in averagesSocialWelfareLists:
            ...
            plt.xticks(simulations,simulations)
            plt.title('How the average Social Welfare of a game increases with N:')
            plt.xlabel('Number of voters')
        ...
        for item in averagesNumberOfIterationsLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of iterations of a game increases with N:')
            plt.xlabel('Number of voters')
        ...
        for item in averagesNumberOfDeviationsLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of deviations in a game increases with N:')
            plt.xlabel('Number of voters')
        ...
        for item in averagesNumberOfCyclesLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of cycles occuring in a game increases with N:')
            plt.xlabel('Number of voters')
        ...
        for item in averageVotersBetterInFinalOutcomeLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average percentage of voters strictly better off with final outcome increases with N:')
            plt.xlabel('Number of voters')
