...
def seqPhragmen_sim(k, A, C):
    ...
    for j in range(1,k):
        scMin = 1000000000
        minCandidate = None #Variable that stores the min cand. for each round.
        for c in restC:
            sc = 0
            ...
            if (sc < scMin):
                minCandidate = c
                scMin = sc
        print("Candidate selected for this round: " + str(minCandidate))
        ...
        print("Load after round: " + str(loads))
        print("C left for next round: " + str(restC))
        print("Temporal W: " + str(W) + "\n")
...
def interative_Through_Candidates_Single_Heuristic(k, W, C, i, A, initialA, f):
    '''For each candidate, the voter i checks wether its utility strictly increases (with respect to the original preferences) by inverting 
...
def faction_Heuristic_Sequential_Change(k, W, C, i, A, faction, initialA, f):
    ...
    if (faction[1] == []):
        ...
        size = len(faction) #Faction[0] is a list of voters (Eg: [1,2]) that belong to the same faction as i
...
def create_Approval_Votes_Spatial_Model(C, N, r):
    A = []
...
if __name__ == "__main__":
    ...
    startC = int(input("Enter the initial m value (m = |C|): "))
    endC = int(input("Enter the final m value: "))
    stepInC = int(input("Enter the step in m: "))

    if(startC > endC):
        print("Initial m cannot be higher than final m!")
        exit()

    k = int(input("Enter committe size (k): "))
    max_rounds = int(input("\nEnter maximum number of rounds per game: "))
    num_voting = int(input("Enter number of simulations done per value of m: "))
    threshold = 0
    heuristics = []
    print("[1 = Threshold, 2 = Iterative Single, 3 = Iterative Up To J, 4 = T-and-E, 5 = Factions(Not done!)]")
    h = int(input("Which heuristic do you want to test? "))    
    upToJ = 0
    variablesToChange = []
    if (h == 1):
        heuristics.extend([1 for j in range(n)])
        thresholds = input("Enter threshold for the heuristic [0,1]: ").split()
        variablesToChange = [float(item) for item in thresholds]
    elif (h == 2):
        heuristics.extend([2 for j in range(n)])
    elif (h == 3):
        heuristics.extend([3 for j in range(n)])
        upToJs = input("How many changes? ").split()
        '''
        CHANGE THIS!
        '''
        for item in upToJs:
            if(int(item) > startC):
                print("J cannot be greater than |C|.")
                exit()
            variablesToChange.append(int(item))
    elif (h == 4):
        heuristics.extend([4 for j in range(n)])
    elif (h == 5):
        print("not done")
        exit()
    if(len(variablesToChange) == 0):
        variablesToChange.append(0) #So that at least one is done
    ...
    for item in variablesToChange:
        ...
        for simN in range(startC, endC+stepInC, stepInC):
            simulations.append(simN)
            f.write("\nTESTING WITH " + str(simN)+ " CANDIDATES:\n")
            f.write("----------------------\n")
            C = []
            for l in range(simN):
                C.append("c" + str(l))
            ...
            for test in range(1,num_voting+1):
                ...
                aux = []
                A = create_Approval_Votes_Spatial_Model(C, N, 0.6)
                ...
                initialUtilityArray = []
                for i in range(n):
                    aux = calculate_Num_Selected_Approval(A[i], W, C)
                    initialUtilityArray.append(aux) 
                    totalInitialSocialWelfare += aux 
                maxUtility = temp[1]
                #Calculate the approvals each candidate got:
                Atemp = np.array(A)
                votesCandidates = np.sum(Atemp, 0)
                ...
                initialTotalUtility = calculate_Total_Utility(W, C, A)
                
                startTime = time.time()
                print("UpToJ = " + str(upToJ))
                ...
                finalUtilityArray = [calculate_Utility(i, initialA, outcome[0], C) for i in range(n)]
            ...
            averageInitialSocialWelfare.append(totalInitialSocialWelfare/num_voting)
            print("m = " + str(simN))
        ...
        averageInitialSocialWelfareLists.append(averageInitialSocialWelfare)
    f.close()

    print(simulations)
    print(averagesDPOALists)
    print(averagesTimesLists)
    print(averagesSocialWelfareLists)
    print(averagesNumberOfIterationsLists)
    print(averagesNumberOfDeviationsLists) 
    print(averagesNumberOfCyclesLists)
    ...
    if(not len(variablesToChange) == 0):
        ...
        for item in averagesDPOALists:
            ...
            plt.title('How average dPOA of a game increases with C:')
        ...
        for item in averagesTimesLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How average time of a game increases with C:')
            plt.xlabel('Number of candidates')
        ...
        for item in averagesSocialWelfareLists:
            j += 1
            plot3 = plt.figure(3)
            plt.plot(simulations, item, label = str(variablesToChange[j]) + " Final SW", marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average Social Welfare of a game increases with C:')
            plt.xlabel('Number of candidates')
        ...
        for item in averagesNumberOfIterationsLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of iterations of a game increases with C:')
            plt.xlabel('Number of candidates')
        ...
        for item in averagesNumberOfDeviationsLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of deviations in a game increases with C:')
            plt.xlabel('Number of candidates')
        ...
        for item in averagesNumberOfCyclesLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average number of cycles occuring in a game increases with C:')
            plt.xlabel('Number of candidates')
        ...
        for item in averageVotersBetterInFinalOutcomeLists:
            ...
            plt.plot(simulations, item, label = variablesToChange[j], marker = 'x')
            plt.xticks(simulations,simulations)
            plt.title('How the average percentage of voters strictly better off with final outcome increases with C:')
            plt.xlabel('Committee size (k)')
            plt.ylabel('Average percentage of voters better off')
        plt.legend()
    else:
        plot1 = plt.figure(1)
        plt.plot(simulations, averagesDPOALists[0], marker = 'x')
        plt.xticks(simulations,simulations)
        plt.title('How average dPOA of a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average Dynamic Price of Anarchy')
        ...
        plot2 = plt.figure(2)
        plt.plot(simulations, averagesTimes, marker = 'x')
        plt.xticks(simulations,simulations)
        plt.title('How average time of a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average time of a game/seconds')
        ...
        plot3 = plt.figure(3)
        plt.plot(simulations, averagesSocialWelfare, marker = 'x', label = 'Final SW')
        plt.xticks(simulations,simulations)
        plt.title('How the average Social Welfare of a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average Social Welfare')
        ...
        plot4 = plt.figure(4)
        plt.plot(simulations, averagesNumberOfIterations, marker = 'x')
        plt.xticks(simulations,simulations)
        plt.title('How the average number of iterations of a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average number of iterations')
        ...
        plot5 = plt.figure(5)
        plt.plot(simulations, averagesNumberOfDeviations, marker = 'x')
        plt.xticks(simulations,simulations)
        plt.title('How the average number of deviations in a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average number of deviations')
        ...
        plot6 = plt.figure(6)
        plt.plot(simulations, averagesNumberOfCycles, marker = 'x')
        plt.xticks(simulations,simulations)
        plt.title('How the average number of cycles occuring in a game increases with C:')
        plt.xlabel('Number of candidates')
        plt.ylabel('Average number of cycles')
