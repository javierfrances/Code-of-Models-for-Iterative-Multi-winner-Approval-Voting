...
def seqPhragmen_sim(k, A, C):
    ...
    for j in range(1,k):
        scMin = 1000000000
        minCandidate = restC[0] #Variable that stores the min cand. for each round.
        for c in restC:
            sc = 0
...
def trial_And_Error_Heuristic(k, W, C, i, A, initialA, f):
    ...
    if(initialUtilityArray[i] == 0): #First round ==> store inital utility
        ...
        if(currentUtility < initialUtilityArray[i] and A[i] != initialA[i]):
            ...
            tempA[i] = initialA[i]
            tempRes = seqPhragmen_sim(k, tempA, C)
...
if __name__ == "__main__":
    ...
    for item in variablesToChange:
        ...
        for simN in range(startN, endN+stepInN, stepInN):
            ...
            heuristics = []
            factionVoters = int(item*simN)
            for i in range(factionVoters):
                heuristics.append(5)
            for i in range(factionVoters, simN):
                heuristics.append(0)
